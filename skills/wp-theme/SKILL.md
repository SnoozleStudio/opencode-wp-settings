...
Standards questions route to the standards skills: `wp-php-standards`,
`wp-js-standards`, `wp-css-standards`, `wp-html-standards` (and
`wp-accessibility-standards` for conformance). Deep GSAP API details come from the
vendored `gsap-*` skills (official greensock/gsap-skills) on GSAP-specific requests;
their guidance is generic — the house integration rules in `docs/frontend-stack.md`
(Lenis, Tempus, reduced-motion, manifest enqueue) override it.
