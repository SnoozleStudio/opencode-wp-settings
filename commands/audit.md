...
Read-only audit. Pass 1 (explore agent, neutral): whole-repo structural audit — 1k-line files, thin wrappers, leaked-logic boundaries, duplicate patterns, dead code. Pass 2 (security-auditor agent): WP attack surface — escaping matrix, SQLi, CSRF, capabilities, REST exposure, secrets in git history, options autoload. Pass 3: docs drift — AGENTS.md vs code, README vs reality, and for THIS repo also the docs/README.md inventory tables vs the filesystem (agents/skills/commands/plugins/templates counts, paths, descriptions) and guide references. Produce a findings report: numbered, with file:line, severity, CONFIRMED/PLAUSIBLE, and concrete fixes. Do not edit anything.
